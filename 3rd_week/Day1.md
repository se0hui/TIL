...
1. 메이븐
    ...
    > 앤트 이후에 나온 자바 빌드 도구<br>
    **라이브러리 및 의존 관리 기능**: 앤트로 불가능했던 라이브러리, 의존성 관리 기능을 가능하게 함.<br>
    **XML 스크립트 기반 관리**: 'pom.xml'파일을 사용하여 의존성을 관리함<br>
    **라이프 사이클 개념 도입**: 업데이트를 통해 라이프 사이클 개념이 도입되었고, 빌드 순서와 같은 프로세스를 정의할 수 있어 빌드 과정이 체계적으로 관리 됨<br>
    **개발자 커뮤니티와 레퍼런스**: 가장 많은 사용자가 사용하는 빌드 도구이며, 다양한 레퍼런스가 존재해 문제 해결과 학습이 용이함

<br>

3. 그레이들
    > 가장 최근에 나온 자바 빌드 도구<br>
    **그루비 문법과 관리의 용이성**: Gradle은 그루비 문법을 사용하고, 스크립트를 'build.gradle'파일에 작성함
    XML 기반 스크립트에 비해 관리가 편리하여 대규모 프로젝트에서 유리함<br>
...
![image.png](https://velog.velcdn.com/images/lcheun/post/65ee3577-6d70-4c50-a026-a7eb9dc4ecb5/image.png)

- **파일 공유 및 별칭**
  - 디렉토리들이 서브 디렉토리들과 파일을 공유하고, 동일한 파일에 대해 서로 다른 별칭(Alias)을 사용할 수 있다.
  - 디스크 공간 절약 및 하나의 파일이 여러 경로 이름을 가질 수 있다.
- **디렉토리 구조의 복잡성**
  - 복잡한 디렉토리 구조로 인해 파일 탐색 시 성능 저하가 발생할 수 있다.
  - 동일한 파일을 여러 경로로 접근 가능하다.
- **파일 삭제와 참조계수**
  - 파일 삭제 시 Dangling Pointer 문제가 발생 가능하다.
  - 참조계수를 사용하여 파일이 참조되지 않을 때만 삭제한다.

<br><br>

### General Graph(일반 그래프) Directory

![image.png](https://velog.velcdn.com/images/lcheun/post/a6f4f59e-d28c-4808-9de3-8a8b4ff536e5/image.png)

- **그래프 구조와 사이클**
  - 트리 구조에 링크를 추가하여 사이클을 허용하는 그래프 구조
  - 사이클로 인해 무한 루프에 빠질 수 있어 일반적으로 사용되지는 않는다.
- **링크 제한과 Garbage Collection**
  - 하위 디렉토리가 아닌 파일에 대한 링크만 허용하거나, Garbage Collection을 통해 전체 파일 시스템을 순회하고 접근 간으한 것만 표시한다.
  - 사용되지 않은 디스크 공간을 회수하고 불필요한 파일을 제거한다.
- **디렉토리와 파일 공유의 유연성**
  - 디렉토리와 파일의 공유에 완전한 융통성을 제공하며, 탐색 알고리즘이 간단하고 파일 및 디렉토리 접근이 용이하다.
- **참조 계수와 디스크 공간 관리**
  - 참조 계수를 사용하여 불필요한 파일을 제거하고 사용 공간을 늘린다.
  - Garbage Collection을 통해 사용되지 않은 디스크 공간을 되찾는다.

<br><br><br><br>

- [x] **Garbage Collection**: 컴퓨터 시스템에서 메모리 관리의 일환으로 사용되지 않는 객체나 데이터, 즉 "쓰레기"를 자동으로 식별하고 제거하는 과정이다. 주로 다음과 같은 역할을 한다.

    1. 메모리 회수: 프로그램이 실행되면서 더 이상 필요없는 메모리 공간을 회수하여 다른 용도로 사용할 수 있도록 함
    2. 메모리 누수 방지: 메모리 누수는 사용하지 않는 메모리가 해제되지 않아 시스템 성능에 악영향을 미치는 현상이며, 이를 방지하여 시스템의 안정성과 성능을 유지한다.
    3. 자동화된 관리: 프로그래머가 직접 메모리 해제를 관리할 필요 없이 자동으로 이루어진다. 수동 메모리 관리의 오류를 줄이고 코드의 복잡성을 줄인다.

<br>

- [x] **"디렉토리와 파일의 공유에 완전한 융통성을 제공"** 의 의미:

    1. 파일과 디렉토리의 자유로운 접근: 동일한 파일이나 디렉토리를 여러 위치에서 접근이 가능하도록 하며, 여러 경로에서 참조할 수 있어 사용자가 필요에 따라 유연하게 파일 시스템을 구성할 수 있다.
    2. 다양한 링크 방식 지원: 파일이나 디렉토리를 다른 디렉토리 내에서 별칭(링크)로 생성할 수 있으며, 심볼릭 링크 또는 하드 링크와 같은 다양한 링크 방식을 지원하여 파일을 다양한 위치에서 재사용할 수 있다.
    3. 유연한 구조: 파일 시스템 구조가 복잡하거나 변경되어도 유연하게 대응할 수 있도록 하며, 특정 디렉토리의 내용이나 파일을 다른 디렉토리와 공유하는 것이 가능하여 파일 고나리와 협업에 있어 높은 유연성을 제공한다.